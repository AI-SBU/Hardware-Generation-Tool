...
void genNetwork(int N, int M1, int M2, int M3, int T, int R, int B, vector<int>& constVector, string modName, ofstream &os) {
   ...
   int P2 = 4; // replace this with your optimized value
   int P3 = 4; // replace this with your optimized value
   ...
   os << "\tlogic signed ["<< T-1 << " : 0] layer1_output_data;" << endl;
   os << "\tlogic unsigned layer1_output_valid;" << endl;
   os << "\tlogic unsigned layer2_input_ready;" << endl;

   os << "\tlogic signed ["<< T-1 << " : 0] layer2_output_data;" << endl;
   os << "\tlogic unsigned layer2_output_valid;" << endl;
   os << "\tlogic unsigned layer3_input_ready;" << endl;

   os << "   // this module should instantiate three layers and wire them together" << endl;

   os << subModName1 << "\tlayer1(.clk(clk), .reset(reset), .input_valid(input_valid), \n" <<
   "\t\t\t\t\t.input_data(input_data), .input_ready(input_ready), .output_data(layer1_output_data), \n" <<
   "\t\t\t\t\t.output_valid(layer1_output_valid), .output_ready(layer2_input_ready));" << endl;    

   os << subModName2 << "\tlayer2(.clk(clk), .reset(reset), .input_data(layer1_output_data), .input_valid(layer1_output_valid), \n" <<
   "\t\t\t\t\t .input_ready(layer2_input_ready), .output_data(layer2_output_data), .output_valid(layer2_output_valid), .output_ready(layer3_input_ready));" << endl;

   os << subModName3 << "\tlayer3(.clk(clk), .reset(reset), .input_data(layer2_output_data), .input_valid(layer2_output_valid), \n" <<
   "\t\t\t\t\t .input_ready(layer3_input_ready), .output_data(output_data), .output_valid(output_valid), .output_ready(output_ready));" << endl;
