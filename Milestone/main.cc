...
void genFCLayer(int M, int N, int T, int R, int P, vector<int>& constVector, string modName, ofstream &os) {
   ...
   os <<
      ...
      << endl << endl;
   //os << "\tlogic unsigned R = " << R << ";" << endl;
   os << "\tlogic unsigned["<< MAddrBits-1 << " : 0] addr_w;" << endl;
   os << "\tlogic unsigned["<< NAddrBits-1 << " : 0] addr_x;" << endl;
   os << "\tlogic signed [" << T - 1 << " : 0] v_out, m_out;" << endl;
   os << "\tlogic unsigned wr_en_x;" << endl;
   os << "\tlogic unsigned clear_acc;" << endl;
   os << "\tlogic unsigned en_acc;" << endl << endl;
   os << "\tcontrolFSM #(" << M << "," << N  << ") controlMod(.clk(clk), .reset(reset), .input_valid(input_valid), .output_ready(output_ready)," << endl;
   os << "\t\t\t\t\t\t\t\t\t.addr_x(addr_x) , .wr_en_x(wr_en_x),.addr_w(addr_w), .en_acc(en_acc), .clear_acc(clear_acc)," << endl;
   os << "\t\t\t\t\t\t\t\t\t.input_ready(input_ready), .output_valid(output_valid));" << endl << endl;

   os << "\tdatapath #(" << T << ", " << R <<") datapathMod(.clk(clk), .reset(reset), .m_out(m_out), .v_out(v_out)," << endl;
   os << "\t\t\t\t\t\t\t\t\t.en_acc(en_acc), .output_data(output_data), .clear_acc(clear_acc), .output_valid(output_valid)," << endl;
